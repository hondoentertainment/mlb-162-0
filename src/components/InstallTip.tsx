...
export function InstallTip() {
  ...
  return (
    <aside className="install-tip" data-testid="install-tip">
      <div>
        <p className="install-tip-title">Add 162-0 to your Home Screen</p>
        {kind === 'install' ? (
      ...
      <div className="install-tip-actions">
        {kind === 'install' && (
          <button type="button" className="btn btn-secondary" data-testid="install-accept" onClick={() => void install()}>
            Install
          </button>
        )}
