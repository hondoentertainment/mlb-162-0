...
export function Encyclopedia() {
  ...
  return (
    <section data-testid="encyclopedia">
      ...
      <p className="lede">
        Search the draft pool. Hide numbers to practice Diamond IQ, or leave them on to see
        Lahman franchise-decade lines.
      </p>
          ...
          {rows.map((p) => (
            <li key={p.id} className="player-card" data-testid={`pedia-${p.id}`}>
              ...
              <div className="badges">
                ...
                {p.statsSource === 'lahman' ? (
                  <span className="badge">Lahman</span>
                ) : (
              ...
              {!hideStats && (
                <div className="stats">
                  {p.pitching ? formatPitching(p) : formatBatting(p)}
                </div>
              )}
            </li>
          ))}
